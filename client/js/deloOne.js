...
function info(isn, type) {
    ...
    $('.popFields').html('<p>Загрузка</p>'); 
        ...
        success: function (data) { 
            if(type=='org'){
                var data=data[0]["Organiz"];
                var html="";
                if(data.NAME    ){html+="<div><b>"+data.NAME+"</b></div>"}
                if(data.FULLNAME){html+="<div><b>Полное наименование: </b><p>"+data.FULLNAME+"</p></div>"}
                ...
                if(data.LAW     ){html+="<div><b>Юридический адрес: </b><p>"+data.LAW+"</p></div>"}
